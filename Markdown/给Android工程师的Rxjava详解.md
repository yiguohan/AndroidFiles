...
RxJava在GitHub主页上的自我介绍是"alibrary for composing asynchronous and event-based programs ovservable sequences for the Java JVM"（一个在 Java VM 上使用可观测的序列来组成异步的、基于事件的程序的库）。这就是 RxJava ，概括得非常精准。
...
Android 创造的 `AsyncTask` 和 `Handler` ，其实都是为了让异步代码更加简洁。RxJava 的优势也是简洁，但它的简洁的与众不同之处在于，**随着程序逻辑变得越来越复杂，它依然能够保持简洁**。

![ä¸¾ä¸ªä¾å­](http://ww4.sinaimg.cn/large/52eb2279jw1f2rx409pcnj2044048mx5.jpg)
...
![OnClickListener è§å¯èæ¨¡å¼](http://ww4.sinaimg.cn/mw1024/52eb2279jw1f2rx42h1wgj20fz03rglt.jpg)
...
![å³ç³»éå¾](http://ww4.sinaimg.cn/mw1024/52eb2279jw1f2rx489robj20lk0a8my2.jpg)
...
![è¿å¹¶æ²¡æä»ä¹diaoç¨](http://ww3.sinaimg.cn/mw1024/52eb2279jw1f2rx4ddcncj2046053gll.jpg)
...
- 在最初的`subscribe()` 中 `onSubscribe` 指的是 `Observable` 中的 `onSubscribe` 对象，这个没有问题，但是 `lift()` 之后的情况就复杂了点。
...
![lift() åçå¾](http://ww1.sinaimg.cn/mw1024/52eb2279jw1f2rxcrna27j20h40d1q4f.jpg)

或者可以看动图：

![lift åçå¨å¾](http://ww4.sinaimg.cn/mw1024/52eb2279jw1f2rxcu9f46g20go0cz4qp.gif)

两次和多次的 `lift()` 同理，如下图：

![ä¸¤æ¬¡ lift](http://ww1.sinaimg.cn/mw1024/52eb2279jw1f2rxcvophmj20h30hl0v3.jpg)
...
![subscribeOn() åç](http://ww4.sinaimg.cn/mw1024/52eb2279jw1f2rxcynbsuj20ha0d7wg2.jpg)

`observeOn()` 原理图：

![observeOn() åç](http://ww4.sinaimg.cn/mw1024/52eb2279jw1f2rxd05lttj20hj0cyabl.jpg)
...
![çº¿ç¨æ§å¶ç»¼åè°ç¨](http://ww1.sinaimg.cn/mw1024/52eb2279jw1f2rxd1vl7xj20hd0hzq6e.jpg)
